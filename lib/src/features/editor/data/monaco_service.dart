...
import 'package:context_collector/src/features/editor/bridge/bridge.dart';
import 'package:context_collector/src/features/editor/bridge/webview_controller.dart';
import 'package:context_collector/src/features/editor/core/model/editor_settings.dart';
import 'package:context_collector/src/features/editor/data/settings_service.dart';
...
class MonacoService extends StateNotifier<EditorStatus> {
  ...
  Future<void> initialize() async {
    ...
    debugPrint(
      '[MonacoService] Full initialization sequence started...',
    );
    ...
    try {
      ...
      if (_queuedContent != null) {
        ...
        await bridge.setContent(
          '',
        ); // Start with empty content if nothing was queued
      }
      ...
      debugPrint(
        '[MonacoService] Fatal error during initialization: $e',
      );
    }
  }
  ...
  Future<String> _prepareHtml(
    String assetPath, {
    required bool forWindows,
  }) async {
    ...
    final finalPath = forWindows
        ? Uri.file(vsPath).toString()
  ...
  Future<void> _copyAssets(String targetDir) async {
    ...
    final assets = manifest.listAssets().where(
      (key) => key.startsWith(_assetBaseDir),
    );
